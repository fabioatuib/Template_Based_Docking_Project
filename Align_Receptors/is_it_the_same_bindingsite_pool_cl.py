...
def main():
    ...
    for index, uniprot_id, ref1, ref2 in dictionary_df[['uniprot_id', 'ref1', 'ref2']].itertuples():
        ...
        work += [(index, path_to_ligang_ref1, path_to_ligand_ref2, path_to_pdb_ref1, path_to_pdb_ref2)]
...
def run_in_parallel(index, path_to_ligang_ref1, path_to_ligand_ref2, path_to_pdb_ref1, path_to_pdb_ref2):
    try:
        ...
        distance = None
        error = e
    return (index, rmsd, distance, error)
...
def align(path_to_ligang_ref1, path_to_ligang_ref2, path_to_pdb_ref1, path_to_pdb_ref2):
    ...
    distances = []
    for at1 in cmd.index("/2gtn_lie///lie"):
        for at2 in cmd.index('/3p7b_p7b///p7b'):
            distances += [cmd.get_distance("%s`%d" % at1, "%s`%d" % at2)]
    ...
    return rmsd, distance #com1_to_com2
